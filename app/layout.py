...
# Try-except around the layout definition to catch and display any errors
try:
    ...
    print(f"Error in layout definition: {e}")
    print(traceback.format_exc())
    # Create a minimal fallback layout for debugging
    error_msg = str(e)
    error_trace = traceback.format_exc()
    layout = dbc.Container([
        html.H1("Error Loading Application", className="text-danger"),
        html.Hr(),
        html.Div([
            html.H3("Error Message:"),
            html.Pre(error_msg, className="bg-light p-3 border"),
            html.H3("Traceback:"),
            html.Pre(error_trace, className="bg-light p-3 border")
        ]),
        html.Hr(),
        html.H3("Environment Info:"),
        html.Pre(f"Current Directory: {os.getcwd()}", className="bg-light p-1"),
        html.Pre(f"Files in current directory: {os.listdir('.')}", className="bg-light p-1"),
        html.Pre(f"Python version: {sys.version}", className="bg-light p-1"),
        html.Pre(f"Dash version: {dash.__version__}", className="bg-light p-1")
    ], fluid=True)
    
    # Create an empty content_layout to avoid further errors
    content_layout = {}